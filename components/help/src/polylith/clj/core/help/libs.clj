...
(defn help [extended? cm]
  (str "  Shows all libraries that are used in the workspace.\n"
       "\n"
       "  poly libs [" (s/key ":compact" cm) "] [" (s/key ":outdated" cm) "] [" (s/key ":transitive" cm) "] [" (s/key ":update" cm) "] [libraries:" (s/key "LIBS" cm) "] [out:" (s/key  "FILENAME" cm) "]\n"
       "\n"
       ...
       "\n"
       "    " (s/key ":transitive" cm) " = Also resolves each project's full dependency tree and adds a\n"
       "                  row per resolved version. A library that a project only gets\n"
       "                  transitively (via another library) is marked with a '+' in that\n"
       "                  project's column, which makes it visible when two projects end up\n"
       "                  on different versions of the same library. Only libraries that are\n"
       "                  declared directly somewhere in the workspace are added. Needs Maven\n"
       "                  metadata (network or a primed ~/.m2), so it is slower than the\n"
       "                  default view.\n"
       "\n"
