...
(defn help-text [color-mode]
  (str "  Validates the workspace.\n"
       ...
       "    Environments are allowed to share names with interfaces, components and bases.\n"
       "\n"
       "  " (color/error color-mode "Error 106") " - Multiple components that share the same interfaces in an environment.\n"
       ...
       "    Library usage for a brick is calculated using " (color/purple color-mode ":ns->lib") " in './deps.edn' for all its namespaces.\n"
       "\n"
       ...
       "    A path is not allowed to exist in both the development environment and a profile.\n"
       ...
       "    A library is not allowed to exist in both development and a profile.\n"
       "    The solution is to remove the library from dev or the profile.\n"
       "\n"
       "  " (color/warning color-mode "Warning 205") " - Reference to missing library in " (color/purple color-mode ":ns->lib") " in ./deps.edn.\n"
       "    Libraries defined in " (color/purple color-mode ":ns->lib") " must also be defined by the environment.\n"
       "\n"
       "  " (color/warning color-mode "Warning 206") " - Reference to missing namespace in " (color/purple color-mode ":ns->lib") " in ./deps.edn.\n"
       "    Namespaces defined in " (color/purple color-mode ":ns->lib") " must also exist in the environment.\n"))
