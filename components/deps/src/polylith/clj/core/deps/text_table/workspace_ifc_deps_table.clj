...
(defn interface-cell [column row interface-name brick-name brick->interface-deps empty-character]
...
(defn interface-column [column interface-name brick-names brick->interface-deps empty-character]
  (concat
    [(text-table/cell column 1 interface-name :yellow :right :vertical)]
    (map-indexed #(interface-cell column (+ %1 3) interface-name %2 brick->interface-deps empty-character)
                 brick-names)))

(defn interface-columns [interface-names brick-names brick->interface-deps empty-character]
  (apply concat (map-indexed #(interface-column (+ (* %1 2) 3) %2 brick-names brick->interface-deps empty-character)
                             interface-names)))
...
(defn table [{:keys [settings interfaces components bases]}]
  (let [{:keys [color-mode empty-character]} settings
        ...
        interface-cols (interface-columns interface-names brick-names brick->interface-deps empty-character)
