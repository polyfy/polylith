...
(def switch-ws-dir-via (c/fn-explorer "dir-via" :switch-ws #'ws-shortcuts/select-dirs))
(def switch-ws-file-via (c/fn-explorer "file-via" :switch-ws #'ws-shortcuts/select-files))

(defn switch-ws [ws-shortcuts]
  (c/single-txt "switch-ws" :switch-ws
                (concat [switch-ws-file switch-ws-dir]
                        (when ws-shortcuts [switch-ws-dir-via switch-ws-file-via]))))
