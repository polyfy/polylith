(ns ^:no-doc polylith.clj.core.info.table.brick.profile-columns
...
(defn column [index profile disk-paths bricks is-show-resources]
  (let [column (+ 503 (* 2 index))
...
(defn columns [bricks profiles disk-paths is-show-resources]
  (apply concat
         (map-indexed #(column %1 %2 disk-paths bricks is-show-resources)
                      profiles)))
