...
(defn table [{:keys [settings profiles projects components bases paths changes workspaces]} is-show-loc is-show-resources]
  (let [{:keys [color-mode thousand-separator]} settings
        ...
        ifc-column (ifc-column/column sorted-components bases ws-components ws-bases)
        brick-column (brick-column/column sorted-components bases ws-components ws-bases alias->workspace changes color-mode)
