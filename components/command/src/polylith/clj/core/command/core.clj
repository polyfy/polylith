...
(defn execute [{:keys [cmd args alias dialect name top-ns branch help is-local more page ws is-tap is-git-add is-github is-commit is-update is-show-brick is-show-workspace is-show-project is-verbose is-fake-poly is-search-for-ws-dir get out interface selected-bricks selected-projects unnamed-args] :as user-input}]
    ...
    (let [brick-name (first selected-bricks)
      ...
      (if ok?
        (case cmd
          ...
          "create" (create/create ws-dir workspace args name alias dialect top-ns interface branch is-git-add is-commit color-mode)
