...
(defn upgrade-lib [ws-dir color-mode entity-type entity-name [lib-name {:keys [type version latest-version]}]]
  (let [filename (str ws-dir "/" entity-type "s/" entity-name "/deps.edn")
        message (str lib-name " to " latest-version " in " (color/entity type entity-name color-mode) ".")]
