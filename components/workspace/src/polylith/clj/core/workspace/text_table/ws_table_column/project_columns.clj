...
(defn column [index {:keys [alias paths]}
              bricks disk-paths alias->bricks-to-test is-show-loc is-show-resources thousand-separator]
...
(defn columns [projects bricks disk-paths {:keys [project-to-bricks-to-test]}
  ...
  (let [project->alias (into {} (map (juxt :name :alias) projects))
    ...
    (apply concat
           (map-indexed #(column %1 %2 bricks disk-paths alias->bricks-to-test is-show-loc is-show-resources thousand-separator)
                        projects))))
