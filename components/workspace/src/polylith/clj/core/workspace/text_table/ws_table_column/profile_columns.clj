...
(defn column [index profile start-column disk-paths settings bricks is-show-resources]
...
(defn columns [start-column bricks profiles disk-paths settings is-show-resources]
  (apply concat
    (map-indexed #(column %1 %2 start-column disk-paths settings bricks is-show-resources)
                 profiles)))
