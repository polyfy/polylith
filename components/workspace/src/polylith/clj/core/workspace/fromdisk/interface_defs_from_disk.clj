...
(defn interface-from-disk [ws-dir top-namespace interface-name {:keys [namespace file-path]} interface-ns]
  (let [content (file/read-file (str ws-dir "/" file-path))
...
(defn defs-from-disk
  ...
  [ws-dir top-namespace interface-name src-namespaces interface-ns]
  (->> src-namespaces
       (filter #(interface-ns? (:namespace %) interface-name top-namespace interface-ns))
       (mapcat #(interface-from-disk ws-dir top-namespace interface-name % interface-ns) )
