...
(defn run-tests-statements [env class-loader test-statements test-namespaces bricks-to-test-msg color-mode]
  (println (str "\nRuning tests for the " (color/environment env color-mode) " environment in " (count test-namespaces) " namespaces: " bricks-to-test-msg))
  ...
  (when (-> test-paths empty? not)
    (let [color-mode (-> workspace :settings :color-mode)
      ...
      (if (-> test-statements empty?)
        ...
        (run-tests-statements name class-loader test-statements test-namespaces bricks-to-test-msg color-mode)))))
