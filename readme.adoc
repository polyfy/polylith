...
NOTE: *Work in Progress*: While we are very proud of our documentation, we are currently working to make it even better! To give you a sneak peek, we built this `SNAPSHOT` release from our https://github.com/polyfy/polylith/tree/issue-318[issue-318] branch.
In the future, we'll build `SNAPSHOT` releases from our https://github.com/polyfy/polylith/tree/master[master] branch.
