...
The _poly_ tool uses its own syntax when passing in parameters. When using the _poly_ command as a Clojure CLI Tool (see xref:install.adoc#_install_as_clojure_cli_tool[Install using Clojure CLI]) we instead use the standard way of passing in arguments as pairs.
