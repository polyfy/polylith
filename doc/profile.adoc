...
Notice here that the profiles contain both `src` and `testv directories.
...
(it's okay to keep `poly/user name`, because it's also the name of the interface that both `user` and `user-remote` share):
...
image::images/profile/compare-with-target-design.png[width=600]
