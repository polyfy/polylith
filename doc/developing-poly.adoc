...
. Run the script `scripts/clojars-stats.bb`, and update the section on this page.
...
. Check that `:template-data` in `workspace.edn` uses the latest version for `:clojure-ver` and `:shadow-cljs-ver`, by executing:
  - `clojure -X:deps find-versions :lib org.clojure/clojure | grep -Eo '"[0-9]+\.[0-9]+\.[0-9]+"' | tail -1`
...
. Execute `bb check-env` followed by bb `gen-all`.
...
. Update version to 0.2.21.SNAPSHOT, commit and push, see xref:#create-a-snapshot-release[make a snapshot release for instructions].
