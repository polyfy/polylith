...
5. Run `bb check` to check for reflection warnings and such.
   - If new bricks, add the source directory to the `:check` alias in `./deps.edn`.
6. Verify that we have included everything in https://github.com/polyfy/polylith/blob/master/next-release.md[next-release.md].
7. Commit and push your changes.
8. Go to the https://github.com/polyfy/polylith[polylith repo] and prepare to merge the `snapshot-022034` branch into `master`, by clicking the "Compare & pull requests" button at the top.
...
10. If the next release is a snapshot release, execute `git switch -c snapshot-022035`.
...
8. Update the doc and its navigation:
...
9. Search for occurrences of `:poly-version:` constants, and set the version to `0.2.20` wherever it appears (at the top of .adoc files).
10. Commit and push your changes.
11. Execute `bb gen-all`. Compare the output with what's in git, and commit and push the changes if everything looks correct.
...
22. Announce in https://clojurians.slack.com/archives/C015AL9QYH1[#releases] in clojurians in Slack, by summarizing the most important changes + the included issues and PRs + add a link to the #polylith channel.
