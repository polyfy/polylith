...
Add a brief line summarizing your change to link:/next-release.txt[next-release.txt].
...
=== `all` - `help`
By default, the xref:commands.adoc[help] command only mentions the xref:commands.adoc#migrate[migrate] command when your workspace requires migration. Specifying the `:all` argument forces the mentioning of `migrate`:
