...
13. Execute `git switch -c snapshot-022101` to create a first snapshot branch for the next release (here we imagine that the next release is 0.2.21).
14. Update the version component:
...
16. Update https://github.com/polyfy/polylith/blob/master/next-release.md[next-release.md]:
- Change the heading to "Included since 0.2.20"
- Clean everything (remove everything under each header)
17. Create a `0.2.21-SNAPSHOT #1` release, see xref:#create-a-snapshot-release[make a snapshot release for instructions].
18. Announce in https://clojurians.slack.com/archives/C015AL9QYH1[#releases] in clojurians in Slack, by summarizing the most important changes + the included issues and PRs + add a link to the #polylith channel.
19. Announce on X (Twitter) and maybe other places.
