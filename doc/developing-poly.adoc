...
...the autocomplete for all available parameters will be activated, including those that are rarely used
(and are mostly in the way) and parameters that only make sense when developing the `poly` tool itself.
...
This parameter is used to fake that we are using a non-snapshot version of the `poly` tool,
and not the `polyx` tool.
...
If the shell is started from the `polylith` repository as in this example, then pages will automatically be opened
in the current branch, and we don't have to pass in `branch:BRANCH`, otherwise the `master` branch will be used.
