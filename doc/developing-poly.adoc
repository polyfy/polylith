...
1. Run all tests: `poly test :all`
2. Update the `version` component and ensure that:
...
3. Ensure the versions are correctly specified in `readme.adoc`:
   - `:is-snapshot` is set to `true`
   - `:snapshot-number:` is set to `34`.
   - `:snapshot-version:` is set to `0.2.20`.
   - `:stable-version:` is set to `0.2.19`.
4. When adding a new doc page, update the _Doc updates_ section in `next-release.md`.
5. Whenever the workspace structure changes:
...
6. If a new attribute is added to _workspace.edn_
...
7. Check that `:template-data` in `workspace.edn` uses the latest version for `:clojure-ver` and `:shadow-cljs-ver`.
8. Run `bb check` to check for reflection warnings and such.
   - If new bricks, add the source directory to the `:check` alias in `./deps.edn`.
9. Execute `bb gen-all`
   - Compare the output with what's in git, and commit the changes if everything looks good.
10. Verify that we have included everything in https://github.com/polyfy/polylith/blob/master/next-release.md[next-release.md].
11. Commit and push your changes.
12. If the next release is a stable release, build a xref:#local-cljdoc[local version] of Cljdoc:
...
13. Go to the https://github.com/polyfy/polylith[polylith repo] and prepare to merge the `snapshot-022034` branch into `master`, by clicking the "Compare & pull requests" button at the top.
If the https://app.circleci.com/pipelines/github/polyfy[circlci pipeline] is green, merge it into the `master` branch.
14. When built, find newly built release under https://github.com/polyfy/polylith/releases[releases]
    - Edit and update it with "0.2.20-SNAPSHOT 34"
    - List included issues and other changes.
15. Execute `git switch master` followed by `git pull` to get the latest from `master`.
16. If the next release is a snapshot release, execute `git switch -c snapshot-022035`, otherwise follow the instructions for creating a stable release.
...
4. Ensure the versions are correctly specified in `readme.adoc`:
   - `:is-snapshot` is set to `false`
   - `:snapshot-number:` is set to `0`.
   - `:stable-version:` is set to `0.2.20`.
5. Whenever the workspace structure changes, remember to update the xref:versions.adoc[Versions] table, and also update the code that migrates exported files in the https://github.com/polyfy/polylith/blob/f5da3d2e206182c7231cabf4207a7001f82508b4/components/ws-file-reader/src/polylith/clj/core/ws_file_reader/from_disk.clj#L11[ws-file-reader] component.
6. Ensure that `readme.adoc` lists the correct sponsors by cross-checking it against the https://github.com/sponsors/polyfy/dashboard/your_sponsors[actual] sponsors.
7. Update the doc and its navigation:
...
8. Search for occurrences of `:poly-version:` constants, and set the version to `0.2.20` wherever it appears (at the top of .adoc files).
9. Commit.
10. Execute `bb gen-all`
    - Compare the output with what's in git
    - Commit and push the changes if everything looks correct.
11. Go to the https://github.com/polyfy/polylith[polylith repo] and prepare to merge the `release-0220` branch into `master`, by clicking the "Compare & pull requests" button at the top.
...
12. Update the https://github.com/polyfy/polylith/releases/tag/v0.2.20[release notes] for the 0.2.20 release:
...
13. Update the https://github.com/polyfy/homebrew-polylith[Polylith Homebrew Tap].
14. Execute `git switch master` followed by `git pull` to get the latest from master.
15. Execute `git switch -c snapshot-022100` to create a first snapshot branch for the next release (here we imagine that the next release is 0.2.21).
16. Update the version component:
    - Update version to 0.2.21 SNAPSHOT #0 (revision to SNAPSHOT, patch to 21, snapshot to 0)
17. Update readme.adoc:
    - set `:is-snapshot` to `true`
    - leave `:snapshot-number:` as 0
    - set `:snapshot-version:` to 0.2.21
18. Update https://github.com/polyfy/polylith/blob/master/next-release.md[next-release.md]:
    - Change the heading to "Included since 0.2.20"
    - Clean everything (remove everything under each header)
19. Create a `0.2.21-SNAPSHOT #0` release, see xref:#create-a-snapshot-release[make a snapshot release for instructions].
20. Delete old `0.2.20-SNAPSHOT` releases.
21. Announce in https://clojurians.slack.com/archives/C015AL9QYH1[#releases] if a minor release, or in https://clojurians.slack.com/archives/C06MAR553[#announcements] if a big release (in clojurians in Slack) by summarizing the most important changes + the included issues and PRs + add a link to the #polylith channel.
