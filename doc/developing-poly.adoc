...
10. Execute `bb gen-all`. Compare the output with what's in git, and commit and push the changes if everything looks correct.
11. Build a xref:#local-cljdoc[local version] of Cljdoc:
...
12. Go to the https://github.com/polyfy/polylith[polylith repo] and prepare to merge the `release-0220` branch into `master`, by clicking the "Compare & pull requests" button at the top.
...
13. Update the https://github.com/polyfy/polylith/releases/tag/v0.2.20[release notes] for the 0.2.20 release:
...
14. Update the https://github.com/polyfy/homebrew-polylith[Polylith Homebrew Tap].
15. Execute `git switch master` followed by `git pull` to get the latest from master.
16. Execute `git switch -c snapshot-022101` to create a first snapshot branch for the next release (here we imagine that the next release is 0.2.21).
17. Update the version component:
    - Update version to 0.2.21 SNAPSHOT #1 (revision to SNAPSHOT, patch to 21, snapshot to 1)
18. Update readme.adoc:
    - set `:snapshot-number:` to 1
    - set `:snapshot-version:` to 0.2.21
19. Update https://github.com/polyfy/polylith/blob/master/next-release.md[next-release.md]:
    - Change the heading to "Included since 0.2.20"
    - Clean everything (remove everything under each header)
20. Create a `0.2.21-SNAPSHOT #1` release, see xref:#create-a-snapshot-release[make a snapshot release for instructions].
21. Announce in https://clojurians.slack.com/archives/C015AL9QYH1[#releases] in clojurians in Slack, by summarizing the most important changes + the included issues and PRs + add a link to the #polylith channel.
