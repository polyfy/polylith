...
5. Check that `:template-data` in `workspace.edn` uses the latest version for `:clojure-ver` and `:shadow-cljs-ver`.
6. Run `bb check` to check for reflection warnings and such.
   - If new bricks, add the source directory to the `:check` alias in `./deps.edn`.
7. Verify that we have included everything in https://github.com/polyfy/polylith/blob/master/next-release.md[next-release.md].
8. Commit and push your changes.
9. Go to the https://github.com/polyfy/polylith[polylith repo] and prepare to merge the `snapshot-022034` branch into `master`, by clicking the "Compare & pull requests" button at the top.
...
11. If the next release is a snapshot release, execute `git switch -c snapshot-022035`.
