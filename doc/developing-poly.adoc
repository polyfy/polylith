...
The poly tool reads starts by reading the workspace into a representation in memory.
...
3. When adding a new doc page, update the _Doc updates_ section in `next-release.md`.
4. Whenever the workspace structure changes:
...
5. If a new attribute is added to _workspace.edn_
...
6. Check that `:template-data` in `workspace.edn` uses the latest version for `:clojure-ver` and `:shadow-cljs-ver`.
7. Run `bb check` to check for reflection warnings and such.
   - If new bricks, add the source directory to the `:check` alias in `./deps.edn`.
8. Verify that we have included everything in https://github.com/polyfy/polylith/blob/master/next-release.md[next-release.md].
9. Commit and push your changes.
10. Go to the https://github.com/polyfy/polylith[polylith repo] and prepare to merge the `snapshot-022034` branch into `master`, by clicking the "Compare & pull requests" button at the top.
...
12. If the next release is a snapshot release, execute `git switch -c snapshot-022035`.
...
Create a `0.2.20-SNAPSHOT 34` release.
