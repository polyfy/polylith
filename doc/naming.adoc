...
If a component does one thing then we can name it based on that, e.g. `validator`, `invoicer` or `purchaser`.
...
API's that are heavily used like `aws` can skip the suffix in some cases.
