...
With start from version `0.2.18` (see issue https://github.com/polyfy/polylith/issues/187#issuecomment-1203567170[187]) not only interfaces matching the name specified in `:interface-ns` in `workspace.edn` will be treated as interfaces, but also the interface names `interface` and `ifc`.
