...
* It sometimes makes sense for a http://clojure-doc.org/articles/language/functions#multi-arity-functions[multi-arity function] in an interface to delegate to a single arity function in the implementing namespace:
+
[source,clojure]
----
(defn foo
...
* It sometimes makes sense for a http://clojure-doc.org/articles/language/functions#variadic-functions[variadic functions] in an interface to delegate to function in the implementing namespace that accepts the variadic portion as a vector:
+
[source,clojure]
----
(defn foo [a b & other]
