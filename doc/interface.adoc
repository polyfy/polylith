...
To give an example, let's pretend we have the interface namespace `user.interface`
...
The "poly" tool not only checks the number of parameters and the types (if we use type hints) in functions and macros,
it also ensures that the names of the parameters match, to minimize the risk of them coming in the wrong order.
