...
0.1.1` and `medley/medley 1.0.0` - plus a `{plus}` for `c` on the existing
`com.stuartsierra/dependency 1.0.0` row.
A `{plus}` means the project only gets that library+version transitively (`x` / `t` are
...
* `project-c`, `project-d` and `dev` all resolve `medley/medley` to `1.0.0`, transitively
  via `brick3`'s `local/lib-a` - even though _nothing_ in the workspace declares
