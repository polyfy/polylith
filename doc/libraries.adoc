...
image::images/libraries/output/libs-libraries-slugger.png[width=600]
...
image::images/libraries/output/libs-libraries-ring-s.png[]

== Validate
[[validate]]

Often you want each library to use the same version.
You can configure the tool to warn if any library is inconsistent, in _workspace.edn_.
...
Here, we use the https://github.com/polyfy/polylith/tree/snapshot-030102/examples/inconsistent-libs[inconsistent-libs] workspace as an example:
...
image::images/libraries/output/inconsistent-libs.png[width=500]
...
image::images/libraries/inconsistent-libs-warning-message.png[width=800]
...
image::images/libraries/output/inconsistent-libs-error.png[width=500]

image::images/libraries/inconsistent-libs-error-message.png[width=800]
...
image::images/libraries/output/inconsistent-libs-exclude.png[width=500]
...
By default, `poly libs` only lists the libraries that bricks and projects _declare_
in their `deps.edn` files.
A library that a project only gets _transitively_ (pulled in by another library) is
not shown for that project, which can hide version drift between projects.

We use the https://github.com/polyfy/polylith/tree/master/examples/transitive-lib-deps[transitive-lib-deps]
workspace as an example.
It has two components, `brick1` and `brick2`, and (besides `development`) two projects,
`project-a` (alias `a`) and `project-b` (alias `b`):
...
* `project-a` contains both bricks, `project-b` contains only `brick1`.
...
image::images/libraries/output/transitive-lib-deps.png[width=600]

`com.stuartsierra/dependency` shows up for `a` and `dev` (which include `brick2`), but
not for `b`,  even though `project-b` _does_ have the library on its classpath,
at version `0.1.1`, pulled in by `brick1` via `com.stuartsierra/component`.
...
image::images/libraries/output/transitive-lib-deps-transitive.png[width=600]

The `com.stuartsierra/dependency 0.1.1` row is now visible, marked with a `+` in the
`b` column.
The `+` means the project only gets that library+version transitively (`x` / `t` are
still used for a directly declared library, `-` for "not on the classpath").
`project-b` runs `0.1.1` while `project-a` and `dev` run `1.0.0`, and both rows are
highlighted because <<validate,inconsistent library versions>> are configured as a
warning in this workspace.

Only libraries that are declared directly somewhere in the workspace are added, to
keep the focus on libraries where a declared and a resolved version can drift apart.

NOTE: `:transitive` needs Maven metadata (network access, or a fully primed
...
https://en.wikipedia.org/wiki/Npm[NPM] dependencies are specified in `package.json` files:
...
Source dependencies go under the `"dependencies"` key, while test dependencies go under the `"devDependencies"` key (see https://github.com/furkan3ayraktar/clojure-polylith-realworld-example-app/blob/cljs-frontend/projects/realworld-frontend/package.json[this] file as an example).
