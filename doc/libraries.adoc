...
image::images/libraries/output/transitive-lib-deps.png[width=700]

`com.stuartsierra/dependency` only shows up for `a` and `dev` (both include `brick2`).
It's missing entirely for `b`, `c` and `d`, even though they _do_ have the library on
...
image::images/libraries/output/transitive-lib-deps-transitive.png[width=700]

Two new rows appear - `com.stuartsierra/component 0.1.0` and
`com.stuartsierra/dependency 0.1.1`, neither of which any brick declares directly -
plus a `x` for `c` on the existing `com.stuartsierra/dependency 1.0.0` row.
...
because <<validate,inconsistent library versions>> are configured as a warning in this
workspace.
