...
Here is a map of ways on how to navigate to different pages with the `doc` command:

image::images/doc/doc-overview.png[]

{nbsp} +

The image:images/gitbook-badge.png[] badge takes us to the documentation for the latest https://github.com/polyfy/polylith/releases[release].

The image:images/cljdoc-snapshot-badge.png[] badge takes us to the latest `SNAPSHOT` build,
which is the one you should use if you work on code in the
https://github.com/polyfy/polylith[master] branch that lives between releases
(the `SNAPSHOT` documentation is built every time we commit something to the `master` branch).
