...
image::images/doc/doc-top.png[width=400]

At the top of the page we have different links to pages that can be good to have access to.

image::images/doc/doc-overview.png[]

{nbsp} +

Worth noticing here is that the image:images/gitbook-badge.png[] badge takes us to the latest final https://github.com/polyfy/polylith/releases[release],
while image:images/cljdoc-snapshot-badge.png[] takes us to the latest `SNAPSHOT` release.
The latter is built every time we push code to the `master` branch, and since a final release almost always includes
more than one issue/commit, the https://cljdoc.org/[cljdoc] based `SNAPSHOT` documentation will be updated multiple times
between final releases, and should be used if you use the latest version of the `poly` command or `clj-poly` library
