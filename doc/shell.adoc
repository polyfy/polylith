...
TIP: Our examples assume you are exploring from our ongoing tutorial `example` xref:workspace.adoc[workspace].
...
NOTE: The xref:commands.adoc#create[create] and xref:commands.adoc#test[test] commands do not work on workspaces you have switched to.
...
NOTE: You can export a workspace to a `.edn` file, e.g. via `poly ws out:ws.edn`.
...
----
$ poly ws-dir:../clojure-polylith-realworld-example-app
                  _      _ + _   _
#####   _ __  ___| |_  _| |-| |_| |_
#####  | '_ \/ _ \ | || | | |  _| ' \
...
[source,text,subs="+attributes"]
----
$ poly ws-file:../usermanager-example/ws.edn
                  _      _ + _   _
#####   _ __  ___| |_  _| |-| |_| |_
#####  | '_ \/ _ \ | || | | |  _| ' \
#####  | .__/\___/_|\_, |_|_|\__|_||_|
...
----
example$ doc help:
check                deps-brick           doc                  switch-ws
create-base          deps-project-brick   info                 tap
create-component     deps-project         libs                 test
create-project       deps-workspace       migrate              version
create-workspace     deps                 overview             ws
create               diff                 shell
----
...
----
* Remote git branches are suggested for some commands for the `branch:` argument (for autocomplete to work here, you need to start your shell with xref:developing-poly.adoc#all[:all] e.g., `poly :all`):
+
[source,text]
----
