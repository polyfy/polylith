...
First you need to configure these in the xref:configuration.adoc#workspaces[:workspaces] key in your xref:configuration.adoc#user[user config] file (`~/.config/polylith/config.edn` in this example):
...
{:color-mode "dark"
 :empty-character "."
 :thousand-separator ","
 :workspaces [{:dir "examples/doc-example" :name "doc"}
              {:dir "examples/multiple-workspaces2/shared"}
...
A workspace that lives in a directory is specified with `:dir`, and an exported workspace is specified with `:file`. They are accessed by the name of the directory/filename, but can be overridden by the `:name` keyword.

Now we can switch workspace from the xref:shell.adoc[shell] with `via-dir:NAME` and `via-file:NAME` where NAME is the name of the directory/filename. This can be overridden by specifying `:name` (as with `doc` in this example):
...
shared (examples/multiple-workspaces2/shared)
