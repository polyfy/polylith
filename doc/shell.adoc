...
Now we can switch workspace from the xref:shell.adoc[shell] with `dir-via:NAME` or `file-via:NAME` where NAME is the name of the directory/file:

[source,text]
----
polylith$ switch-ws dir-via:
...
[source,text]
----
polylith$ switch-ws dir-via:doc
dir:doc-example$
----
...
[source,text]
----
polylith$ switch-ws file-via:another-ws
file:another-ws
----
