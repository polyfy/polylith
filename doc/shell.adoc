...
=== Switching to a predefined workspaces ===

If you have workspaces xref:workspace.adoc#sub-workspaces[within] your workspace, other cloned workspaces, or  workspaces xref:explore-the-workspace.adoc#export-workspace[exported to file] there is a convenient way to access these from the xref:shell.adoc[shell].
If you want to follow along in this example, you first need to clone the https://github.com/polyfy/polylith[polylith] repo somewhere outside the example workspace.

First you need to configure these in the xref:configuration.adoc#ws-shortcuts[:ws-shortcuts] key in your xref:configuration.adoc#user[user config] file (`~/.config/polylith/config.edn` in this example):
...
{:color-mode "dark"
 :empty-character "."
 :thousand-separator ","
 :ws-shortcuts [{:dir "examples/doc-example" :name "doc"}
...
A workspace that lives in a directory is specified with `:dir`, and an exported workspace is specified with `:file`. They are accessed by the name of the directory/filename, but can be overridden by the `:name` keyword (as with `doc` in this example).

Now we can switch workspace from the xref:shell.adoc[shell] with `via-dir:NAME` and `via-file:NAME` where NAME is the name of the directory/filename:
...
test-runners            (examples/test-runners)
----

If we type `b` and press kbd:[Tab] followed by kbd:[Enter] we switch to the `backend` workspace directory:

[source,text]
----
polylith$ switch-ws via-dir:backend
dir:backend$
----

As before, we can always switch back to the root workspace with:
[source,text]
----
...
Workspaces that has been exported to files with `out:FILENAME`, e.g. `poly ws out:anotherws.edn` can be selected with:

[source,text]
----
polylith$ switch-ws via-file:
anotherws (../sandbox/anotherws.edn)
----

We can now switch `anotherws`:
[source,text]
----
polylith$ switch-ws via-file:anotherws
file:anotherws
----

This is equivalent to `switch-ws file:../sandbox/anotherws.edn`.
