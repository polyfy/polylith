...
If we press `<tab>`, it will complete the info command and suggest available parameters:
...
The exception is `:project` which has to start with `:p` to distinguish it from `project`:
...
The brick parameter lists all bricks, including "no brick" represented by a dash (if `-` is chosen,
then no bricks will be selected for testing):

image::images/shell/shell-info-brick.png[width=600]

A command that really benefits from being executed from the shell is the xref:commands.adoc#ws[ws] command.
...
When starting a shell from a workspace directory (where the `workspace.edn` file lives)
...
We can now switch to that workspace using the xref:commands.adoc#switch-ws[switch-ws] command in combination with the `dir` parameter:
...
We can select the clojure-polylith-realworld-example-app directory by typing `.` followed by `<tab>`
to select `..` and then typing `clo` (or something long enough) followed by `<tab>` to select the workspace:
...
The `dir:` prefix indicates that commands will be executed from the selected workspace,
and not from the current directory. We can go back to the workspace directory by executing `switch-ws`:

image::images/shell/shell-switch-ws.png[width=600]

If a workspace has been exported to a file using something like `poly ws out:ws.edn` then we can switch to that workspace.
Here we use the https://github.com/seancorfield/usermanager-example/tree/polylith[usermanager] project as an example
(make sure to switch to the `polylith` branch, after cloning the repository):

image::images/shell/shell-switch-ws-file.png[width=600]

The prompt name is picked from the `:name` attribute in the `ws.edn` file and is prefixed with `file:`
to indicate that it has been imported from a file.

The "trick" the shell does is to append `ws-dir:DIR` or `ws-file:FILE`
to the command for us before the command is executed
(if we have switched workspace using the `dir:DIR` or `file:FILE` parameters).

=== Start a shell in another workspace

It's possible to start a shell and immediately switch to a workspace
(now the xref:commands.adoc#shell[shell] command has to be included), e.g.:
...
...or read a workspace from a file, e.g.:
...
* The workspace structure can be browsed via the xref:commands.adoc#ws[ws] command.

* The documentation can be browsed via the xref:commands.adoc#doc[doc] command.
...
- If a `project:X` parameter is selected from the info or test command,
...
* If we want to go to the user home directory we have to type `~/` and not just `~`
...
We often execute the xref:commands#create-workspace[create workspace] command outside a xref:workspace.adoc[workspace].
The `workspace` parameter is therefore not suggested by xref:commands.adoc#create[create] when inside an existing workspace,
but if we are outside we will get some guidance:

image::images/shell/shell-create-workspace.png[width=600]

=== Rarely used parameters

Rarely used xref:parameters.adoc[parameters] are not suggested by the autocomplete, e.g. `color-mode` and `branch`.
A way to include them is to pass in `:all` when starting a shell:
