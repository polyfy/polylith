...
We can now switch to that workspace using the xref:commands.adoc#switch-ws[switch-ws] command in combination with the _dir_ parameter:
...
We can select the clojure-polylith-realworld-example-app directory by typing `.` followed by _<tab>_
to select `..` and then typing `clo` (or something long enough) followed by _<tab>_ to select the workspace:
...
Here we use the https://github.com/seancorfield/usermanager-example/tree/polylith[usermanager] project
as an example (make sure to switch to the _polylith_ branch, after cloning the repository):

image::images/shell/shell-switch-ws-file.png[width=600]

The promt name is picked from the _:name_ attribute in the _ws.edn_ file and is prefixed with _file:_
...
It's possible to start a shell and immediately switch to a workspace (now the _shell_ command has to be included), e.g.:
...
* Tag patterns defined in _workspace.edn_ are suggested when using the `since:X` parameter.
...
but if we are outside a workspace, the autocomplete will help out with creating a workspace, e.g.:

image::images/shell/shell-create-workspace.png[width=600]

=== Rarely used parameters

Rarely used xref:parameters.adoc[parameters] are not suggested by the autocomplete, e.g. _color-mode_.
A way to include them is to pass in _:all_ when starting a shell:
