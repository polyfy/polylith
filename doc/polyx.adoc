...
If we want to use `polyx`, then we need to add it as an alias to `./deps.edn`, e.g. by copying the existing `:poly` alias and change `:deps/root` from "projects/poly" to "projects/polyx":
