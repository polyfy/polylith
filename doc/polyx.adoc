...
If we want to use `polyx`, then we need to add it as an alias to `./deps.edn`:
...
This will create the `info.png` image containing the output from the info command.
...
then an image will be created in the specified format, but if it ends with `.txt` then a text file is created:
