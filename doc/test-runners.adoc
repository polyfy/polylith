...
<2> And xref:#imre-kaocha-test-runner[Imre Imre Kószó's Kaocha test runner] as another example. Don't forget to configure the kaocha-wrapper at the project level, see the documentation or https://github.com/polyfy/polylith/tree/prepare-0219/examples/test-runners[this] example project.
...
../projects/baz/config.edn:
[source,clojure]
----
{:alias "baz"
 ; Use both the poly default and the external test runner for this project
 :test {:create-test-runner [:default
