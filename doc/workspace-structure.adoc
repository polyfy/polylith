...
If ignored, then xref:validations.adoc#error111[Error 111 - Unreadable namespace in brick/project] will not be triggered.
...
{:alias "user-s",
 ...
 :config-filename "examples/doc-example/projects/user-service/deps.edn",
 :deps {"user" {:src {}, :test {}},
...
** `:keep-lib-versions` A vector containing the library names (as symbols) that should not be affected when executing `poly libs :update`.
** `:necessary` If we get xref:validations.adoc#error111[Error 111 - Unreadable namespace in brick/project] and know that the brick(s) has to be included in the project, then we can add the necessary bricks(s) to the project in a vector for this key.
** `:test`
