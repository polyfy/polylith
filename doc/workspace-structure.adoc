...
* `:indirect-lib-deps` Only present when `:transitive` is passed (e.g. `poly libs :transitive`).
Holds the Maven libraries that end up on the project's resolved classpath but are _not_ declared by the project or any of its bricks (i.e. pulled in transitively via another library), so that a declared version and a transitively resolved version of the same library can be compared across projects.
** `:src` A map with the transitively resolved libraries, following the same structure as `:lib-deps` > `:src` (`:type`, `:version`, `:size`).
