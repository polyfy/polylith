...
A vector containing the workspaces, other than the root workspace, recursively retrieved using the workspace directories specified by xref:configuration.adoc#ws-workspaces[workspaces]].

These workspaces are complete, except from these attributes:
* `:projects` > PROJECT > `:bricks-to-test`
* `:projects` > PROJECT > `:projects-to-test`

The attribute `:ws-relative-dir` is also added, which is a relative path to the root workspace directory, `ws-dir`, also listed at the top by the xref:commands.adoc#info[info] command (if any).
