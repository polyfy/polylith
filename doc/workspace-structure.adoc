...
* `:keep-lib-versions` A vector containing the library names that should not be affected when executing `poly libs :update`. Configured in the base's config.edn file.
...
* `:alias` The alias that is specified in `:projects` in `workspace.edn` for this project. Configured in the project's config.edn file.
