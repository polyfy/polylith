...
which is the same structure that all the `poly` commands operate on internally.
...
or "SNAPSHOT" if a snapshot release (older releases can have other values here).
** `:snapshot` Only set if a snapshot release, otherwise 0. The first snapshot release will have the value of 1.
