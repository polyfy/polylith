...
* `:api`
** `:breaking` Increased by one if introducing a non-backward compatible change of the {cljdoc-api-url}[API].
** `:non-breaking` Increased by one when a non-breaking change is made to the {cljdoc-api-url}[API].
* `:release`
