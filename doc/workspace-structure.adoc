...
* `:fake-sha` Set to _c91fdad_ if _fake-sha:c91fdad_ is given. Used to set the "stable since" sha for the output from the _info_ command. Used to control the output when taking screenshots for the documentation.
...
* `:replace` Used in tests, when maintaining the _poly_ tool. Set to e.g. _[{:from "/Users/joakimtengstrand", :to "my-home"}]_ if _replace:$HOME:my-home_ is given. It will replace this text too, so the output will actually be _[{:from "my-home", :to "my-home"}]_ which is the behavior we want (the whole idea with the replace parameter is to get the same output every time).
