...
command is also added to git, which will come in handy in this example.

====
NOTE: If we leave _:auto-add_ as _false_, then we have to manually add files that are created with the
xref:commands.adoc#create-component[create component], xref:commands.adoc#create-base[create base],
and xref:create-project[create project] commands, to xref:git.adoc[git].
In that case we can add them to git by passing in _:git-add_.
====
...
There is also a fourth way, and that is to execute the command from a shell.

We can start an interactive xref:commands#shell[shell] by leaving out the command in the fist two cases (or include "shell")
but in the third it has to include "shell":
