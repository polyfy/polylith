...
There is also a shorter form that works in the first two cases:
...
Let's continue by adding the `ser` component to `./deps.edn`:
...
It specifies that it has a `srcv, `resourcesv and `test` directory.
