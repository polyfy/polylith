...
The xref:commands.adoc#test[test] command will not be executed if xref:commands.adoc#check[check] discovers any errors.
...
Which will ignore all files with the name "test_setup.clj" or files that end with "/test-setup.clj" in the `command-line` project.
