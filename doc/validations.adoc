...
If you know what you're doing, you can suppress warnings and errors via the `:disable` key
...
Each entry in the `:disable` vector selects one validation, using `:warning` for warnings
(the 2xx codes) and `:error` for errors (the 1xx codes).
...
If you leave both `:bricks` and `:projects` out, the validation is suppressed everywhere.

Most validations can be suppressed this way, except for errors 105, 108 and 110, and message 301 has its own
xref:libraries.adoc[configuration] via `:inconsistent-lib-versions` instead.

The `:disable` entries are not validated, so a misspelled key or an unsupported validation
number is silently ignored (the validation keeps running).
...
=== Suppress more than one validation
