...
* The {cljdoc-api-url}/polylith.clj.core.api.interface[api] interface (only these exposed functions are part of the public API,
all other code is subject to change).

* The {cljdoc-api-url}/polylith.clj.core.test-runner-contract.interface[test-runner-contract] interface.

You can add `clj-poly` as a dependency to a `:deps` or `:extra-deps` in your `deps.edn` file
