...
Instead of waiting for the next version to be released, we can set the latest commited SHA from the
https://github.com/polyfy/polylith/commits/master[master branch] in `./deps.edn`, each time it updates (see the example above).
This works especially well if we also start a xref:commands.adoc#shell[shell] from the workspace root.
====
...
The `poly` functionality can be accessed as a library, which is explained link:{cljdoc-api-url}/polylith[here].
