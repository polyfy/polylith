...
To install the _poly_ tool on Mac, execute:
...
====
CAUTION: do not use the _install-latest_ option to `clojure -Ttools` as it does not support _:deps/root_.
====
...
Once we have added one of the aliases above, we can now use the _poly_ tool from the terminal, e.g.:
...
If we use the tools.deps CLI to execute the _poly_ tool, e.g. `clojure -M:poly test`,
