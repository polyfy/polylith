...
TIP: From the build pipeline there is an option to return an error code if there are no tests to run:
[source,shell]
----
poly test :fail-if-nothing-to-test
----
(you need to start a shell with e.g. `poly :all` to enable auto-complete for `:fail-if-nothing-to-test`)
