...
// TODO-being (from Lee Read): Moved from dependencies.adoc, re-edit and integrate when reviewing this doc
// == Notes on `ws-dir`

// Before we continue, it is relevant to mentioning that most commands, except the xref:commands.adoc#test[test] and xref:commands.adoc#create[create] commands, can be executed from other workspaces by specifying the `ws-dir` argument, e.g.:
...
// Another way of specifying the workspace dir is the `::` argument, this set the workspace dir to the first parent directory that contains a `workspace.edn` file, e.g.:

// [source,shell]
// ----
// cd projects/realworld-backend # <1>
// poly info :: # <2>
// ----
