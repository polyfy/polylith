...
This will give the exact same output as if we execute `poly info` on the machine that created _ws.edn_. All commands except _test_ and _create_ can be executed when _ws-file_ or _ws-dir_ is given. If we run commands in a shell, then _switch-ws file:ws.edn_ will add _ws-file:ws.edn_ to all subsequent commands to get the same result.
