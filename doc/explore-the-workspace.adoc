...
An even better way of exploring the data structure is to do it from a _shell_ which we have already covered in the xref:shell[Shell] section.
...
An alternative way to reach the same result is to turn off the coloring and pipe to _ws.edn_:
...
To load this workspace, they have to give the _ws-file_ parameter, e.g.:
...
This will give the exact same output as if we execute `poly info` on the machine that created _ws.edn_.
All commands except _test_ and _create_ can be executed when _ws-file_ or _ws-dir_ is given.
If we run commands in a shell,
then _switch-ws file:ws.edn_ will add _ws-file:ws.edn_ to all subsequent commands to get the same result.
