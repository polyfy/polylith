...
From the _:src_ context, components can only depend on interfaces,
but bases can depend on both component interfaces and any namespace in a brick,
e.g. another base (if you have more than one base in the project).

Components are allowed to access other components test code from its _:test_ context.
Bases are allowed to access any test code in components and bases.
Accessing other bases' test code from a base is fine,
but accessing other components' test code has the disadvantage
that it will make it more difficult to swap out a component for another,
because it may break code that depends on that test code.

A better strategy is to create a separate test-helper component,
put the shared code there, and depend on that component's interface.
