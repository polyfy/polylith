...
The `poly` tool introduces restrictions on how we can access namespaces of other bricks:

[%autowidth]
|===
| Entity | Restrictions from the `:src` context | Restrictions from the `:test` context
...
and the other is that the `poly` tool doesn't force us to only use interfaces from project code.
...
but in that case it's probably better to put all code in a single base,
