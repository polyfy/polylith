...
[source,text]
----
  poly 0.3.0 (2025-06-24) - https://github.com/polyfy/polylith
  ...
  Example:
    ...
    poly create workspace name:myws top-ns:com.my.company dialects:cljs :commit
    poly create workspace name:myws top-ns:com.my.company dialects:cljs branch:master :commit
    poly deps
...
----
  ...
  poly libs [:compact] [:outdated] [:update] [libraries:LIBS] [out:FILENAME]
    ...
    :update   = Updates all libraries to the latest version (in 'deps.edn'
                and 'package.json' files). If LIBS is given, then only update selected
