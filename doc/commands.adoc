...
[source,text]
----
  Opens a document in a web browser.

  poly doc [ARG]

    ARG = (omitted)       -> Opens the poly tool readme.

          command:COMMAND -> Opens the help for the given poly COMMAND,
                             or the overall help if not given.

          page:PAGE       -> Opens the given PAGE for the poly tool documentation.

          ws:KEY          -> Opens the workspace structure section in the poly tool
                             documentation, and scrolls to the KEY section.

          more:TYPE       -> Opens up the given blog-post, podcast, video, et cetera.
