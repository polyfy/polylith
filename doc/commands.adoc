...
[source,text]
----
  poly 0.2.19 (2024-02-02) - https://github.com/polyfy/polylith
...
----
  ...
  Warning 206 - Deprecated workspace.
    Triggered if brick and project configuration are still stored in workspace.edn
    instead of local config.edn files.
  ...
  Warning 207 - Unnecessary components were found in project.
    ...
    name in the :necessary vector for a project in its config.edn file. See an example
...
----
  ...
  poly libs [:compact] [:outdated] [:update] [libraries:LIBS] [out:FILENAME]
    ...
    :update   = Updates all libraries to the latest version. If LIBS is given,
                ...
                :keep-lib-versions for bricks and projects in their config.edn files.
