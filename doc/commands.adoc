...
[source,text]
----
  poly 0.3.33 (2026-09-11) - https://github.com/polyfy/polylith
...
----
  ...
  Warnings can be suppressed via the :validations > :disable key in
  ...
  Warning 207 - Unnecessary components were found in project.
    ...
    components, put their names in the :necessary vector for a project in :projects
...
----
  ...
  poly libs [:compact] [:outdated] [:transitive] [:update] [libraries:LIBS] [out:FILENAME]

    :compact  = Shows the table in a more compact way.

    :outdated = Shows the latest version of each library, or blank if up to date.

    :transitive = Also resolves each project's full dependency tree and adds a
